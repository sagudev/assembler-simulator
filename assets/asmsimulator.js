...
;app.service('assembler', ['opcodes', function (opcodes) {
    return {
        go: function (input) {
            ...
            //input regex: (I|i)([0-1][0-6]|[0-9])
            //output regex: (O|o)(\d\d|\d)

            var disk_reg_test = function(adr) {
                return disk_reg.test(adr);
            }

            var i_reg_test = function(adr) {
                return /(I|i)([0-1][0-6]|[0-9])/.test(adr);
            }

            var o_reg_test = function(adr) {
                return /(O|o)(\d\d|\d)/.test(adr);
            }
            // MATCHES: "(+|-)INTEGER"
            var regexNum = /^[-+]?[0-9]+$/;
            // MATCHES: "(.L)abel"
            ...
            var parseRegOrNumber = function (input, typeReg, typeNumber) {
                ...
                if (register !== undefined) {
                    ...
                    if (label !== undefined && !disk_reg_test(label)) {
                        ...
                        } else if (o_reg_test(input) && (value < 0 || value > 97)) {
                            throw typeNumber + " must have a value between 0-96";
                        } else if (i_reg_test(input) && (value < 0 || value > 17)) {
                            throw typeNumber + " must have a value between 0-16";
                        } else {
            ...
            var getValue = function (input) {
                ...
                } else if (o_reg_test(input)) {
                    return {type: "number", value: input};
                } else if (i_reg_test(input)) {
                    return {type: "number", value: input};
                } else {
            ...
            for (i = 0, l = code.length; i < l; i++) {
                if (!angular.isNumber(code[i])) {
                    if (!disk_reg_test(code[i]) && !i_reg_test(code[i]) && !o_reg_test(code[i])) {
                        //debugger;
                        if (code[i] in labels) {
        ...
        step: function() {
            ...
            //output regex: (O|o)(\d\d|\d)
                    if (/(D|d)(\d\d\d\d|\d\d\d|\d\d|\d)/.test(base)) {
                        ...
                        base = "d";
                        //debugger;
                    } else if (/(I|i)([0-1][0-6]|[0-9])/.test(base)) {
                        console.log(base + " + " + offset);
                        var ase;
                        ase = base.slice(1);
                        offset = parseInt(Number(ase)) + parseInt(Number(offset));
                ...
                switch(instr) {
                    ...
                    //
                        ...
                        if (/(D|d)(\d\d\d\d|\d\d\d|\d\d|\d)/.test(ra)) {
                            ...
                            //debugger;
                        } else if (/(I|i)([0-1][0-6]|[0-9])/.test(ra)) {
                            ra = ra.slice(1);
                            ra = parseInt(Number(ra)) + 1;
                            ra = "i" + ra;
                            //console.log("end ra is: " + ra);
                            //debugger;
                        } else if (/(O|o)(\d\d|\d)/.test(ra)) {
                            ra = ra.slice(1);
                            ra = parseInt(Number(ra)) + 1;
                            ra = "o" + ra;
                            //console.log("end ra is: " + ra);
...
;app.service('memory', [function () {
    var memory = {
        ...
        load: function (address) {
            ...
            } else if (/(I|i)([0-1][0-6]|[0-9])/.test(address)) {
                if (address < 0 || address >= self.isave.length) {
                    throw "Memory access violation at " + address;
                }
    
                return self.isave[address.slice(1)];
            } else if (/(O|o)(\d\d|\d)/.test(address)) {
                if (address < 0 || address >= self.osave.length) {
                    throw "Memory access violation at " + address;
                }
    
                return self.osave[address.slice(1)];
            } else {
        ...
        store: function (address, value) {
            ...
            if (/(D|d)(\d\d\d\d|\d\d\d|\d\d|\d)/.test(address)) {
                ...
                self.isave[address.slice(1)] = value;
            } else if (/(O|o)(\d\d|\d)/.test(address)) {
                if (address < 0 || address >= self.osave.length) {
                    throw "Memory access violation at " + address;
                }
                self.osave[address.slice(1)] = value;
            } else {
                if (address < 0 || address >= self.data.length) {
                    throw "Memory access violation at " + address;
                }
        ...
        reset: function (disk = 0, inpot = 1, outpot = 1) {
            var self = this;

            if (inpot) {
                for (var i = 0, l = self.isave.length; i < l; i++) {
                    self.isave[i] = 0;
                }
            }

            if (outpot) {
                for (var i = 0, l = self.osave.length; i < l; i++) {
                    self.osave[i] = 0;
                }
            }
...
;app.controller('Ctrl', ['$document', '$scope', '$timeout', 'cpu', 'memory', 'assembler', function ($document, $scope, $timeout, cpu, memory, assembler) {
    ...
    $scope.getMemoryCellCss_o = function (index) {
        return 'output-bg';
    };

    $scope.getMemoryCellCss_i = function (index) {
        return '';
    };
  ...
  fr.readAsText(files.item(0));
    ...
    $scope.inputerf = function () {
        ...
        //memory.store(adr, st);
        memory.store("i0", $scope.inputer1);
        memory.store("i1", $scope.inputer2);
        memory.store("i2", $scope.inputer3);
        memory.store("i3", $scope.inputer4);
        memory.store("i4", $scope.inputer5);
        memory.store("i5", $scope.inputer6);
        memory.store("i6", $scope.inputer7);
        memory.store("i7", $scope.inputer8);
        memory.store("i8", $scope.inputer9);
        memory.store("i9", $scope.inputer10);
        memory.store("i10", $scope.inputer11);
        memory.store("i11", $scope.inputer12);
        memory.store("i12", $scope.inputer13);
        memory.store("i13", $scope.inputer14);
        memory.store("i14", $scope.inputer15);
        memory.store("i15", $scope.inputer16);
    };

    $scope.ttled = function (index) {
        index = "d" + index;
        return $scope.ttle(index);
    };

    $scope.ttle_i = function (index) {
        index = "i" + index;
        return $scope.ttle(index);
    };

    $scope.ttle_o = function (index) {
        index = "o" + index;
        return $scope.ttle(index);
    };
    ...
    $scope.ttle = function (index) {
        ...
        if (index === cpu.gpr[0] && $scope.displayA) {
            ...
            return 'R16';
        } else if (index === cpu.gpr[21] && $scope.displayR32) {
            return 'R17';
        } else if (index === cpu.gpr[22] && $scope.displayR32) {
            return 'R18';
        } else if (index === cpu.gpr[23] && $scope.displayR32) {
            return 'R19';
        } else if (index === cpu.gpr[24] && $scope.displayR32) {
            return 'R20';
        } else if (index === cpu.gpr[25] && $scope.displayR32) {
            return 'R26';
        } else if (index === cpu.gpr[27] && $scope.displayR32) {
            return 'R22';
        } else if (index === cpu.gpr[28] && $scope.displayR32) {
            return 'R23';
        } else if (index === cpu.gpr[29] && $scope.displayR32) {
            return 'R24';
        } else if (index === cpu.gpr[30] && $scope.displayR32) {
            return 'R25';
        } else if (index === cpu.gpr[31] && $scope.displayR32) {
            return 'R26';
        } else if (index === cpu.gpr[32] && $scope.displayR32) {
            return 'R27';
        } else if (index === cpu.gpr[33] && $scope.displayR32) {
            return 'R28';
        } else if (index === cpu.gpr[34] && $scope.displayR32) {
            return 'R29';
        } else if (index === cpu.gpr[35] && $scope.displayR32) {
            return 'R30';
        } else if (index === cpu.gpr[36] && $scope.displayR32) {
            return 'R31';
        } else if (index === cpu.gpr[37] && $scope.displayR32) {
            return 'R32';
        } else {
            return '';
        }
    ...
    $scope.getMemoryInnerCellCss_i = function (index) {
        //ttle();
        //debugger;
        index = "i" + index;
    //    $scope.inputerf();
    return $scope.getMemoryInnerCellCss(index)

    };

    $scope.getMemoryInnerCellCss_o = function (index) {
        //ttle();
        //debugger;
        index = "o" + index;
    //    $scope.inputerf();
    ...
    //    $scope.inputerf();
        if (index === cpu.ip) {
            ...
            return 'marker marker-R17';
        } else if (index === cpu.gpr[22] && $scope.displayR32) {
            return 'marker marker-R18';
        } else if (index === cpu.gpr[23] && $scope.displayR32) {
            return 'marker marker-R19';
        } else if (index === cpu.gpr[24] && $scope.displayR32) {
            return 'marker marker-R20';
        } else if (index === cpu.gpr[25] && $scope.displayR32) {
            return 'marker marker-R26';
        } else if (index === cpu.gpr[27] && $scope.displayR32) {
            return 'marker marker-R22';
        } else if (index === cpu.gpr[28] && $scope.displayR32) {
            return 'marker marker-R23';
        } else if (index === cpu.gpr[29] && $scope.displayR32) {
            return 'marker marker-R24';
        } else if (index === cpu.gpr[30] && $scope.displayR32) {
            return 'marker marker-R25';
        } else if (index === cpu.gpr[31] && $scope.displayR32) {
            return 'marker marker-R26';
        } else if (index === cpu.gpr[32] && $scope.displayR32) {
            return 'marker marker-R27';
        } else if (index === cpu.gpr[33] && $scope.displayR32) {
            return 'marker marker-R28';
        } else if (index === cpu.gpr[34] && $scope.displayR32) {
            return 'marker marker-R29';
        } else if (index === cpu.gpr[35] && $scope.displayR32) {
            return 'marker marker-R30';
        } else if (index === cpu.gpr[36] && $scope.displayR32) {
            return 'marker marker-R31';
        } else if (index === cpu.gpr[37] && $scope.displayR32) {
            return 'marker marker-R32';
        } else {
            return '';
        }
    };
